...
if __name__ == "__main__":
    ...
    for machine in machines:
        full_article_list = [(article["text"], machine) for article in
                             DatabaseGenArticles.get_articles_by_method(machine)]
        training_data += full_article_list[:int(len(full_article_list) * 0.8)]
        if len(full_article_list) >= minarticles:
            print(f"chose language model: {machine}")
    ...
    os.makedirs(STYLOMETRY_DIR, exist_ok=True)
    character_distribution.to_csv(os.path.join(STYLOMETRY_DIR, f"char_distribution{nfeatures}.csv"))
    semantic_distribution.to_csv(os.path.join(STYLOMETRY_DIR, f"sem_distribution{nfeatures}.csv"))
