...
def write_test_distributions():
    ...
    char_features = list(pd.read_csv(os.path.join(STYLOMETRY_DIR, "char_distribution.csv")).columns)[1:]
    sem_features = [eval(feature) for feature in list(pd.read_csv(os.path.join(STYLOMETRY_DIR, "sem_distribution.csv")).columns)[1:]]
    ...
    full_char_distribution.to_csv(os.path.join(STYLOMETRY_DIR, "test_char_distribution.csv"))
    full_sem_distribution.to_csv(os.path.join(STYLOMETRY_DIR, "test_sem_distribution.csv"))

def char_model_prediction(inp):
    models = {}
    for author in authors:
        with open(os.path.join(STYLOMETRY_DIR, author + "_char.pickle"), "rb") as fp:
    ...
    for i in range(inp.shape[0]):
        ...
        human = any(confidence_values[author][i][1] > 0.2 for author in authors[2:])
...
def sem_model_prediction(inp):
    models = {}
    for author in authors:
        with open(os.path.join(STYLOMETRY_DIR, author + "_sem.pickle"), "rb") as fp:
            models[author] = pickle.load(fp)
    confidence_values = {}
    for author in authors:
        confidence_values[author] = models[author].predict_proba(inp)
...
def char_performance():
    test_dataframe = pd.read_csv(os.path.join(STYLOMETRY_DIR, "test_char_distribution.csv"))
    correct_class = []
...
def sem_performance():
    test_dataframe = pd.read_csv(os.path.join(STYLOMETRY_DIR, "test_sem_distribution.csv"))
    correct_class = []
    for i in range(test_dataframe.shape[0]):
        if test_dataframe.iloc[i]["author"] in authors[:2]:
            correct_class.append(1)
